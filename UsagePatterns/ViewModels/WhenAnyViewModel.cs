...
{
    public abstract class WhenAnyViewModel : OperationViewModel, INotifyPropertyChanged
    {
        ...
        protected WhenAnyViewModel(IPathService pathService = null) : base(pathService) { }
        ...
        {
            var reportingFile = CurrentlyReportingFiles[fileInfo.FullName];
            ...
            CurrentlyReportingFiles.TryAdd(fileInfo.FullName, (reportingFile.index, reportingFile.hasFinished, value));
        }
        ...
        protected List<Task<string>> CreateFixedBatchOfCopyTasks(Queue<string> filePaths)
        {
            return Enumerable.Range(0, BatchLevel)
                .SelectMany(i =>
                {
                    ...
                    CurrentlyReportingFiles.TryAdd(fp, (i, hasFinished: false, percent: 0));
